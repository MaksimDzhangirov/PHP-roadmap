...
Безопасность контейнеров – важнейший аспект внедрения и управления контейнерными технологиями, такими как Docker. Он включает в себя набор практик, инструментов и технологий, предназначенных для защиты контейнерных приложений и инфраструктуры, в которой они работают. В этом разделе мы обсудим некоторые ключевые вопросы безопасности контейнеров, лучшие практики и рекомендации.

#### Изоляция контейнера

Изоляция имеет решающее значение для обеспечения надежности и безопасности контейнерных сред. Контейнеры должны быть изолированы друг от друга и от хост-системы, чтобы предотвратить несанкционированный доступ и минимизировать потенциальный ущерб в случае, если злоумышленнику удастся взломать один контейнер.

* **Пространства имен**: Docker использует технологию пространств имен для предоставления изолированных сред для запуска контейнеров. Пространства имен ограничивают то, что контейнер может видеть и получать доступ в более широкой системе, включая процессы и сетевые ресурсы.

* **Cgroups**: контрольные группы (`cgroups`) используются для ограничения ресурсов, потребляемых контейнерами, таких как ЦП, память и ввод-вывод. Правильное использование `cgroups` помогает предотвратить DoS-атаки и ситуации исчерпания ресурсов.

#### Типовые решения и методы обеспечения безопасности

Внедрение лучших практик и конкретных типовых решений обеспечения безопасности во время разработки, развертывания и эксплуатации контейнеров имеет важное значение для поддержания безопасного окружения.

* **Наименьшие привилегии**: контейнеры следует запускать с наименьшими возможными привилегиями, предоставляя только минимальные разрешения, необходимые для приложения.
* **Неизменяемая инфраструктура**: контейнеры следует рассматривать как неизменяемые единицы – после создания их нельзя изменять. Любое изменение должно осуществляться путем развертывания нового контейнера из обновленного образа.
* **Контроль версий**: образы должны находиться в	системе контроля версий и храниться в безопасном реестре контейнеров.

#### Настройка контроля доступа с точки зрения безопасности

Контроль доступа должен применяться как к управлению контейнерами, так и к данным контейнеров, чтобы защитить конфиденциальную информацию и поддерживать общий уровень безопасности.

* **Управление контейнерами**: используйте управление доступом на основе ролей (RBAC), чтобы ограничить доступ к платформам управления контейнерами (например, Kubernetes) и гарантировать, что пользователи имеют только минимальные необходимые права.

* **Данные контейнера**: шифруйте данные при хранении и передаче, особенно при работе с конфиденциальной информацией.

#### Комплекс мероприятий, связанных с уязвимостями контейнеров

Контейнеры могут быть уязвимы для атак, поскольку их образы зависят от множества пакетов и библиотек. Чтобы снизить эти риски, комплекс мероприятий, связанных с уязвимостями, должен быть включен в жизненный цикл контейнера.

* **Сканирование образов**: используйте инструменты автоматического сканирования для выявления уязвимостей в контейнерах и образах. Эти инструменты следует интегрировать в процесс разработки, чтобы выявить потенциальные риски до того, как они попадут в продакшен.

* **Безопасные базовые образы**: используйте минимальные и безопасные базовые образы для создания контейнеров, уменьшая пространство для атаки и потенциальные уязвимости.

* **Регулярные обновления**: поддерживайте актуальность базовых образов и контейнеров с помощью последних исправлений и обновлений безопасности.

Понимая и применяя эти ключевые аспекты безопасности контейнеров, позволит вам не сбиться с пути обеспечения защиты ваших контейнерных приложений и инфраструктуры от потенциальных угроз.

### Меры безопасности для защиты образа

Безопасность образов – важнейший аспект развертывания Docker контейнеров в вашем окружении. Очень важно убедиться, что используемые вами образы безопасны, актуальны и не содержат уязвимостей. В этом разделе мы рассмотрим лучшие практики и инструменты для защиты ваших Docker образов и управления ими.

#### Используйте надежные источники образов

При извлечении образов из общедоступных репозиториев всегда используйте надежные официальные образы в качестве отправной точки для ваших контейнерных приложений. Официальные образы проверяются Docker и регулярно выходят обновления с исправлениями, касающимися безопасности. Вы можете найти эти образы в Docker Hub или других доверенных реестрах.

* Официальные образы: [https://hub.docker.com/explore/](https://hub.docker.com/explore/).

При загрузке образов, собранных другими пользователями или создании собственных всегда проверяйте источник и просматривайте Dockerfile и другие предоставленные файлы, чтобы убедиться, что они соответствуют рекомендациям и не содержат уязвимостей.

#### Поддерживайте актуальность образов

Постоянно отслеживайте актуальность своих образов и регулярно обновляйте их. Это помогает свести к минимуму воздействие известных уязвимостей, поскольку обновления часто содержат исправления безопасности.
...
#### Используйте минимальный по размеру базовый образ

Минимальный базовый образ содержит только самое необходимое для запуска контейнерного приложения. Чем меньше компонентов присутствует в базовом образе, тем меньше простраство для атаки на потенциальные уязвимости.

Примером минимального базового образа является дистрибутив Alpine Linux, который обычно используется в Docker образах из-за его небольшого размера и средств защиты.

* Alpine Linux: [https://alpinelinux.org/](https://alpinelinux.org/)

#### Сканирование образов на наличие уязвимостей

Регулярно сканируйте свои образы на наличие известных уязвимостей с помощью таких инструментов, как Clair или Anchore. Эти инструменты могут обнаруживать потенциальные риски в ваших образах и конфигурациях контейнеров, позволяя устранять их перед отправкой образов в реестр или развертыванием их в продакшене.

#### Подписывайте и осуществяйте проверку образов

Чтобы обеспечить целостность и подлинность ваших образов, всегда подписывайте их с помощью Docker Content Trust (DCT). DCT использует цифровые подписи, чтобы гарантировать, что образы, которые вы извлекаете или отправляете, соответствуют вашим ожиданиям и не были подделаны при передаче.

Включите DCT для вашего Docker окружения, установив следующую переменную окружения:
...
Многоэтапные сборки позволяют использовать несколько инструкций `FROM` в одном файле Dockerfile. Каждый этап может иметь разный базовый образ или набор инструкций, но только последний этап определяет содержимое окончательного образа. Используя многоэтапные сборки, вы можете минимизировать размер и сложность конечного образа, снижая риск появления уязвимостей.

Вот пример Dockerfile с использованием многоэтапных сборок:
...
Следуя этим рекомендациям по обеспечению безопасности образов, вы сможете минимизировать риск уязвимостей и обеспечить безопасность своих контейнерных приложений.

### Меры безопасности во время выполнения

Меры безопасности во время выполнения направлены на обеспечение безопасности Docker контейнеров во время их работы на продакшене. Это критический аспект безопасности контейнеров, поскольку угрозы могут появиться или быть обнаружены после развертывания ваших контейнеров. Надлежащие меры безопасности во время выполнения помогают минимизировать ущерб, который может быть нанесен в случае использования уязвимости.

#### Принцип наименьших привилегий

Убедитесь, что ваши контейнеры придерживаются принципа наименьших привилегий, то есть они должны иметь только минимальные права, необходимые для выполнения намеченных функций. Это может помочь ограничить потенциальный ущерб в случае взлома контейнера.

* По возможности запускайте свои контейнеры от имени пользователя без полномочий root.
* Избегайте запуска привилегированных контейнеров, которые имеют доступ ко всем ресурсам хоста.
* Используйте возможности Linux, чтобы удалить ненужные права доступа из ваших контейнеров.

#### Файловые системы только для чтения

Настроив файловые системы ваших контейнеров только для чтения, вы можете предотвратить изменение критических файлов злоумышленниками или установку вредоносного ПО внутри ваших контейнеров.

* Используйте флаг `--read-only` при запуске контейнеров, чтобы сделать их файловые системы доступными только для чтения.
* Реализуйте монтирование томов или `tmpfs` монтирование для ситуаций, когда требуется разрешение на запись.

#### Сканирование и мониторинг безопасности

Убедитесь, что ваши контейнеры регулярно сканируются на наличие уязвимостей, как в самих образах, так и в среде выполнения.

* Используйте инструменты сканирования контейнеров для обнаружения и исправления уязвимостей в ваших образах.
* Внедрите мониторинг во время выполнения для обнаружения и реагирования на события, связанные с безопасностью, такие как попытки несанкционированного доступа или неожиданный запуск процессов.

#### Изоляция ресурсов

Изолируйте ресурсы ваших контейнеров, такие как ЦП, память и сеть, чтобы предотвратить влияние одного скомпрометированного контейнера на другие контейнеры или хост-систему.

* Используйте встроенные в Docker ограничения ресурсов, чтобы ограничить ресурсы, которые могут потреблять ваши контейнеры.
* Используйте сегментацию сети и брандмауэры, чтобы изолировать ваши контейнеры и ограничить их взаимодействие.

#### Ведите лог с регистрацией выполняемых действий

Ведите лог с регистрацией активности контейнеров, чтобы было проще реагировать на инциденты, устранять неполадки и обеспечивать соответствия требованиям.

* Используйте возможности ведения логов в Docker для сбора логов контейнера и вывода их в централизованное решение для логирования.
* Внедрите инструменты анализа логов для отслеживания подозрительной активности и автоматического оповещения в случае обнаружения потенциального инцидента.

Сосредоточив внимание на мерах безопасности во время выполнения, вы можете гарантировать, что ваши Docker контейнеры будут оставаться в безопасности даже после того, как они будут развернуты в вашей среде. Стремитесь свести к минимуму потенциальное пространство для атаки и постоянно отслеживайте угрозы, чтобы защитить критически важные приложения и данные.

## Docker CLI

Docker CLI (интерфейс командной строки) – это мощный инструмент, который позволяет взаимодействовать и управлять контейнерами, образами, томами и сетями Docker. Он предоставляет пользователям широкий спектр команд для создания, запуска и управления контейнерами Docker и другими ресурсами Docker в рабочих процессах разработки и продакшена.

В этом разделе мы углубимся в некоторые ключевые аспекты Docker CLI, включая следующее:

#### 1. Установка

Чтобы начать работу с Docker CLI, вам необходимо установить Docker на вашем компьютере. Вы можете следовать официальному руководству по установке для вашей операционной системы из [документации Docker](https://docs.docker.com/get-docker/).

#### 2. Основные команды

Вот несколько основных команд Docker CLI, с которыми стоит ознакомиться:

* `docker run`: создать и запустить контейнер из Docker образа
* `docker container`: список запущенных контейнеров
* `docker image`: список всех доступных образов в вашей системе
* `docker pull`: извлечь образ из Docker Hub или другого реестра
* `docker push`: отправить образ в Docker Hub или другой реестр
* `docker build`: создать образ из файла Dockerfile
* `docker exec`: запустить команду в работающем контейнере
* `docker logs`: показать логи контейнера

#### 3. Параметры `docker run`

`docker run` – одна из самых важных команд в Docker CLI. Вы можете настроить поведение контейнера, используя различные параметры, такие как:

* -d, --detach: запустить контейнер в фоновом режиме
* -e, --env: задать переменные окружения для контейнера
* -v, --volume: Bind монтирование тома
* -p, --publish: сделать доступным порт контейнера на хосте
* --name: назначить имя контейнеру
* --restart: указать правила перезапуска контейнера
* --rm: автоматически удалять контейнер при выходе

#### 4. Dockerfile

Dockerfile – это скрипт, содержащий инструкции по созданию Docker образа. Вы можете использовать интерфейс командной строки Docker для создания, обновления и управления Docker образами с помощью Dockerfile.

Вот пример простого Dockerfile:

```
# Задаём базовый образ для использования
FROM alpine:3.7
...
Docker Compose – это CLI инструмент для определения и управления приложениями Docker из нескольких контейнеров с использованием YAML файлов. Он работает вместе с Docker CLI, предлагая согласованный способ управления несколькими контейнерами и их зависимостями.

Установите Docker Compose, используя [официальное руководство по установке](https://docs.docker.com/compose/install/), а затем вы сможете создать файл `docker-compose.yml` для определения и запуска приложений из нескольких контейнеров:
...
В заключение отметим, что Docker CLI – это надежный и универсальный инструмент для управления всеми аспектами контейнеров и ресурсов Docker. Ознакомившись с его командами и возможностями, вы будете готовы с легкостью разрабатывать, поддерживать и развертывать приложения с помощью Docker.

### Образы

Docker образы – это легковесные, автономные и исполняемые пакеты, включающие все необходимое для запуска приложения. Эти образы содержат все необходимые зависимости, библиотеки, среду выполнения, системные инструменты и код, обеспечивающие единообразную работу приложения в различных окружениях.

Docker образы создаются и управляются с помощью Dockerfiles. Dockerfile – это скрипт, состоящий из инструкций по созданию Docker образа, предоставляющий пошаговое руководство по настройке среды приложения.

#### Работа с Docker образами

Docker CLI предоставляет несколько команд для управления Docker образами и работы с ними. Одними из наиболее важных команд являются:

* `docker image ls`: выводит список всех доступных образов в вашей локальной системе
* `docker build`: создаёт образ из Dockerfile
* `docker image rm`: удаляет один или несколько образов
* `docker pull`: извлекает образ из реестра (например, Docker Hub) в вашу локальную систему
* `docker push`: отправляет образ в репозиторий

Например, чтобы получить официальный образ Ubuntu из Docker Hub, вы можете запустить следующую команду:

```
docker pull ubuntu:latest
```

После извлечения образа вы можете создать и запустить контейнер, используя этот образ, с помощью команды `docker run`:

```
docker run -it ubuntu:latest /bin/bash
```

Эта команда создаёт новый контейнер и запускает интерактивный сеанс внутри контейнера с использованием оболочки `/bin/bash`.

#### Совместное использование образов

Docker образы можно публиковать и распространять с помощью реестров контейнеров, таких как Docker Hub, Google Container Registry или Amazon Elastic Container Registry (ECR). Как только ваши образы будут помещены в реестр, другие смогут легко получить к ним доступ и использовать их.

Чтобы поделиться своим образом, сначала необходимо добавить к нему тег соответствующий следующему формату:

```
docker tag <image-id> <username>/<repository>:<tag>
```

Затем вы можете отправить помеченный тегом образ в реестр, используя команду:

```
docker push <username>/<repository>:<tag>
```

В заключение отметим, что Docker образы являются важной частью экосистемы Docker, позволяя разработчикам упаковывать свои приложения, легко ими делиться и обеспечивать единообразность в различных окружениях. Понимая как работают Docker образы и команды для управления ими, вы сможете использовать возможности контейнеризации и улучшить свой процесс разработки.

### Контейнеры

Контейнеры можно рассматривать как легковесные, автономные и исполняемые пакеты программного обеспечения, которые включают в себя все необходимое для запуска элемента программного обеспечения, включая код, среду выполнения, библиотеки, переменные окружения и файлы конфигурации. Контейнеры изолируют программное обеспечение от его окружения, обеспечивая его единообразную работу в различных средах.

#### Почему стоит использовать контейнеры?

* **Переносимость**: контейнеры обеспечивают единообразную работу приложений на разных платформах, будь то ноутбук разработчика или продакшен сервер. Это устраняет проблему «это работает на моей машине».

* **Эффективность**: контейнеры легковесные, поскольку используют общие ресурсы без накладных расходов, присущих полноценной операционной системе. Это позволяет ускорить время запуска и сократить использование ресурсов.

* **Масштабируемость**: контейнеры можно легко масштабировать в зависимости от рабочей нагрузки, что делает их идеальными для распределенных приложений и микросервисов.

* **Единообразность**: контейнеры позволяют разработчикам, QA и командам эксплуатации иметь единообразную среду на протяжении всего жизненного цикла приложения, что приводит к более быстрым и плавным процессам развертывания.

* **Безопасность**: контейнеры обеспечивают определённый уровень изоляции от других контейнеров и базовой хост-системы, что помогает поддерживать безопасность приложений.

#### Работа с контейнерами с помощью Docker CLI

Docker CLI предоставляет несколько команд, которые помогут вам создавать контейнеры, управлять ими и взаимодействовать с ними. Некоторыми наиболее часто используемыми командами являются:

* `docker run`: используется для создания и запуска нового контейнера
* `docker container ls`: выдаёт список запущенных контейнеров
* `docker container stop`: останавливает работающий контейнер
* `docker container rm`: удаляет остановленный контейнер
* `docker exec`: выполняет команду внутри запущенного контейнера
* `docker logs`: извлекает логи контейнера, что полезно для отладки при возникновении проблем

### Тома

Docker тома – это механизм сохранения данных, созданных и используемых Docker контейнерами. Они позволяют отделить данные от самого контейнера, упрощая резервное копирование, миграцию и управление вашими постоянными данными.

#### Почему тома важны

Docker контейнеры по своей природе эфемерны, то есть их можно легко остановить, удалить или заменить. Хотя это отлично подходит для разработки и развертывания приложений, это создает проблемы при работе с постоянными данными. Вот тут-то и приходят на помощь тома. Они предоставляют возможность хранить данные и управлять ими отдельно от жизненного цикла контейнера.

#### Типы томов

В Docker существует три типа томов:

* **Тома хоста**: они хранятся в файловой системе хост-компьютера, обычно в каталоге `/var/lib/docker/volumes`. К ним легко получить доступ, но могут возникнуть проблемы с переносимостью или совместимостью файловой системы.

* **Анонимные тома**: они создаются автоматически, когда контейнер запускается без указания тома. Их идентификатор генерируется Docker, и они также хранятся в файловой системе хост-компьютера.

* **Именованные тома**. Подобно анонимным томам, именованные тома хранятся в файловой системе хост-компьютера. Однако вы можете указать собственное имя, чтобы на него можно было легко ссылаться в других контейнерах или для резервных копий.

#### Управление томами с помощью Docker CLI

Docker CLI предоставляет различные команды для управления томами:

* `docker volume create`: создает новый том с заданным именем
* `docker volume ls`: выводит список всех томов в системе
* `docker volume inspect`: предоставляет подробную информацию о конкретном томе
* `docker volume rm`: удаляет том
* `docker volume prune`: удаляет все неиспользуемые тома

Чтобы использовать том в контейнере, вы можете воспользоваться флагом `-v` или `--volume` во время команды запуска Docker. Например:

```
docker run -d --name my-container -v my-named-volume:/var/lib/data my-image
```

Эта команда создает новый контейнер с названием «my-container», используя образ «my-image», и монтирует том «my-named-volume» по пути `/var/lib/data` внутри контейнера.

### Сети

Сети Docker предоставляют важный способ управления взаимодействием контейнеров. Это позволяет контейнерам передавать данные друг другу и хост-компьютеру, используя различные сетевые драйверы. Понимая и используя различные типы сетевых драйверов, вы можете проектировать контейнерные сети с учетом конкретных ситуаций или требований приложений.

#### Сетевые драйверы

В Docker доступно несколько сетевых драйверов. Здесь мы рассмотрим четыре наиболее распространенных из них:

* **bridge** (**мост**): сетевой драйвер по умолчанию для контейнеров. Он создает частную сеть, в которой контейнеры могут взаимодействовать друг с другом и с хост-компьютером. Контейнеры в этой сети могут получать доступ к внешним ресурсам через сеть хоста.

* **host** (**хост**): этот драйвер удаляет сетевую изоляцию и позволяет контейнерам совместно использовать сеть хоста. Это полезно в ситациях, когда производительность сети имеет решающее значение, поскольку сводит к минимуму накладные расходы на работу контейнерной сети.
 
* **none** (**отключён**): этот сетевой драйвер отключает работу контейнерной сети. Контейнеры, использующие этот драйвер, работают в изолированном окружении без доступа к сети.

* **overlay** (**наложение**): этот сетевой драйвер позволяет контейнерам, развернутым на разных хостах, взаимодействовать друг с другом. Он предназначен для работы с Docker Swarm и идеально подходит для развертывания контейнеров на нескольких хостах или в кластерах.

#### Управление сетями Docker

Docker CLI предоставляет различные команды для управления сетями. Вот несколько полезных команд:

* Вывести список всех сетей: `docker network ls`
* Просмотреть информацию о сети: `docker network inspect <network_name>`
* Создать новую сеть: `docker network create --driver <driver_type> <network_name>`
* Подключить контейнеры к сети: `docker network connect <network_name> <container_name>`
* Отключить контейнеры от сети: `docker network disconnect <network_name> <container_name>`
* Удалить сеть: `docker network rm <network_name>`

## Простота и скорость разработки

До сих пор мы обсуждали только использование Docker для развертывания приложений. Однако Docker также является отличным инструментом для разработки приложений. Существует несколько различных рекомендаций, которые вы можете использовать, чтобы улучшить свой опыт разработки.

* Используйте `docker-compose` в своем приложении для упрощения разработки.
* Используйте bind монтирования кода из вашей локальной системы в файловую систему контейнера, чтобы избежать необходимости пересобирать образ контейнера при каждом отдельном изменении.
* Для автоматической перезагрузки вы можете использовать такие инструменты, как [**vite**](https://vitejs.dev/) для клиентской части, [**nodemon**](https://nodemon.io/) для `nodejs` или [**air**](https://github.com/cosmtrek/air) для `golang`.
* Подумайте о том как вы будете отлаживать ваши приложения. Например, изучите [**delve**](https://github.com/go-delve/delve) для Go, включите отладку в `node.js` с помощью флага
  --inspect и т. д. Неважно, что вы используете, но суть в том, что у вас должен быть способ отладки вашего приложения, работающего внутри контейнера.
* У вас должна быть возможность запускать тесты внутри контейнера. Например, у вас может быть отдельный файл `docker-compose` для запуска тестов.
* У вас должен быть CI конвейер для продакшен образов.
* Эфемерная среда для каждого запроса на включение в проект сделанных вами изменений.
...
Развертывание контейнеров – важнейший шаг в использовании Docker и контейнеризации, позволяющий более эффективно управлять приложениями, легко масштабировать их и обеспечивать стабильную производительность в разных окружениях. В этом разделе представлен обзор того, как развертывать Docker контейнеры для создания и запуска приложений.

#### Введение

Docker контейнеры – это легкие, портативные и самодостаточные окружения, в которых можно запускать приложения и их зависимости. Развертывание контейнеров включает в себя запуск, управление и масштабирование этих изолированных окружений для обеспечения бесперебойной работы ваших приложений.

#### Преимущества развертывания контейнера

* **Единообразность**: контейнеры позволяют вашему приложению одинаково работать в различных окружениях, избегая распространенной проблемы «оно работает на моей машине».
* **Изоляция**: каждый контейнер работает в изолированном окружении, что позволяет избежать конфликтов с другими приложениями и обеспечить независимое управление каждым сервисом.
* **Масштабируемость**: контейнеры упрощают масштабирование приложений за счет запуска нескольких экземпляров и распределения рабочей нагрузки между ними.
* **Контроль версий**: развертывание контейнеров помогает вам управлять различными версиями вашего приложения, позволяя при необходимости легко вернуться к предыдущим версиям.

#### Ключевые понятия

* **Образ**: Docker образ – это легкий автономный исполняемый пакет, который содержит все необходимое для запуска программного обеспечения, включая код, среду выполнения, системные инструменты, библиотеки и настройки.
* **Контейнер**: Docker контейнер – это работающий экземпляр Docker образа. Вы можете развернуть несколько контейнеров из одного образа, каждый из которых будет работать независимо.
* **Docker Реестр**: место, где хранятся и откуда извлекаются Docker образы. Docker Hub – это реестр по умолчанию, используемый Docker, но при желании вы можете использовать свой собственный приватный реестр.

#### Шаги по развертыванию контейнеров

* **Создайте Dockerfile**: Dockerfile – это скрипт с инструкциями по созданию Docker образа. В нём должны быть указаны базовый образ, код приложения, зависимости и конфигурации, необходимые для запуска вашего приложения.
* **Создайте Docker образ**: используя Docker клиент, вы можете создать новый образ, запустив `docker build` и указав путь к вашему Dockerfile. Это создаст новый Docker образ на основе инструкций в вашем Dockerfile.
* **Отправьте Docker образ**: после создания образа необходимо отправить его в реестр (например, Docker Hub), чтобы его можно было легко извлечь при развертывании контейнеров. Используйте команду `docker push`, за которой следует название и тег образа.
* **Развертывание контейнера**: чтобы развернуть новый контейнер из Docker образа, используйте команду `docker run`, за которой следует название и тег образа. Это запустит новый контейнер и выполнит необходимое приложение.
* **Управление контейнером**: развертывание включает в себя обеспечение правильной работы контейнера, управление масштабированием, обновлениями и другими ключевыми аспектами. Используйте Docker команды, такие как `docker ps` (для вывода списка запущенных контейнеров), `docker stop` (для остановки контейнера) и `docker rm` (для удаления контейнера), чтобы управлять развернутыми контейнерами.
* **Мониторинг и логирование**: собирайте логи и отслеживайте производительность развернутых контейнеров, чтобы обеспечить их оптимальную работу. При необходимости используйте такие команды, как `docker logs` (для просмотра логов) и `docker stats` (для просмотра статистики контейнера).

#### Заключение

Развертывание контейнеров с помощью Docker позволяет улучшить единообразность, безопасность и масштабируемость приложений, одновременно упрощая управление и сокращая накладные расходы, обычно связанные с развертыванием. Поняв концепции и шаги, изложенные в этом руководстве, вы будете хорошо подготовлены к развертыванию своих приложений с использованием Docker контейнеров.

### PaaS альтернативы

Платформа как услуга (PaaS) – это модель облачных вычислений, которая упрощает развертывание контейнеров и управление ими. Он абстрагирует базовую инфраструктуру, позволяя разработчикам сосредоточиться на создании и запуске своих приложений. Ниже приведены некоторые популярные варианты PaaS для развертывания контейнеров:

#### Amazon Elastic Container Service

[Amazon Elastic Container Service](https://aws.amazon.com/ecs/) – это сервис оркестрации контейнеров с большим количеством настроек, предлагаемый Amazon Web Services. Он позволяет запускать контейнеры без необходимости управлять серверами или кластерами. Он интегрируется с другими сервисами AWS, такими как IAM, CloudWatch и CloudFormation.

* Поддерживает Docker контейнеры и Amazon ECR
* Предлагает возможность бесплатного использования для новых пользователей
* Поддерживает несколько вариантов развертывания
* Платите за то, что используете, без предварительных затрат

#### Google Cloud Run

[Google Cloud Run](https://cloud.google.com/run) – это полностью настраиваемая вычислительная платформа от Google, которая позволяет запускать контейнеры без сохранения состояния. Она предназначен для запуска приложений, которые могут масштабироваться автоматически, что позволяет вам платить только за те ресурсы, которые вы действительно используете.

* Автоматически масштабируется в зависимости от потребностей
* Поддерживает пользовательские домены и TLS сертификаты
* Интегрируется с другими сервисами Google Cloud
* Предлагает бесплатный тариф с большими возможностями

#### AWS Elastic Beanstalk

[AWS Elastic Beanstalk](https://aws.amazon.com/elasticbeanstalk/) – это сервис оркестрации, предлагаемый Amazon Web Services, который позволяет развертывать приложения, управлять ими и масштабировать их с помощью контейнеров, не беспокоясь о базовой инфраструктуре.

* Поддерживает несколько языков и платформ, включая Docker контейнеры
* Интеграция с другими AWS сервисами, такими как RDS, S3 и CloudFront
* Предлагает возможности мониторинга и логирования
* Платите за то, что используете, без предварительных затрат

#### Microsoft Azure Container Instances

[Azure Container Instances](https://azure.microsoft.com/en-us/services/container-instances/) – это служба Microsoft Azure, которая упрощает развертывание контейнеров с использованием бессерверной модели. Вы можете запускать контейнеры, не заботясь о настройке базовой инфраструктуры хостинга или оркестрации контейнеров.

* Быстрый и простой процесс развертывания
* Настраиваемая конфигурация размера, сети и хранилища
* Интеграция со службами Azure и службой Azure Kubernetes
* Модель посекундной оплаты

#### IBM Cloud Code Engine

[IBM Cloud Code Engine](https://www.ibm.com/cloud/code-engine) – это полностью настраиваемая бессерверная платформа IBM, на которой выполняются ваши контейнерные приложения и исходный код. Она поддерживает развертывание, запуск и автоматическое масштабирование приложений в Kubernetes.

* Построена на базе Kubernetes и Knative
* Развертывание из реестра контейнеров или хранилища исходного кода
* Поддерживает событийно-ориентированные и пакетные рабочие нагрузки
* Модель оплаты за использованные ресурсы

При выборе варианта PaaS для развертывания контейнеров учитывайте такие факторы, как интеграция с существующими инструментами, простота использования, стоимость, масштабируемость и поддержка языков программирования и платформ, с которыми знакома ваша команда. Независимо от вашего выбора, варианты PaaS позволяют разработчикам легко развертывать приложения, не беспокоясь об управлении и обслуживании базовой инфраструктуры.

### Kubernetes

Kubernetes (K8s) – это платформа оркестрации с открытым исходным кодом, используемая для автоматизации развертывания, масштабирования и управления контейнерными приложениями. В то время как Docker предоставляет среду выполнения контейнера, Kubernetes расширяет эту функциональность с помощью мощного и гибкого фреймворка для управления ими.

#### Ключевые идеи

* **Кластер**: набор машин, называемых узлами, на которых выполняются контейнерные приложения в Kubernetes. Кластер может иметь несколько узлов для балансировки нагрузки и отказоустойчивости.

* **Узел**: рабочая машина (физическая, виртуальная или облачная), на которой работают контейнеры как часть кластера Kubernetes. Каждый узел управляется мастером Kubernetes.

* **Pod**: самый маленький и простой модуль в объектной модели Kubernetes. Pod представляет собой один экземпляр запущенного процесса и обычно включает один или несколько контейнеров (например, Docker контейнер).

* **Сервис**: абстракция, определяющая логический набор podов и политику доступа к ним. Сервисы обеспечивают балансировку нагрузки, мониторинг и сетевые возможности для соответствующих podов.

* **Deployment**: объект высокого уровня, описывающий желаемое состояние контейнерного приложения. Deploymentы управляют процессом создания, обновления и масштабирования модулей на основе указанного образа контейнера.

#### Зачем использовать Kubernetes?

Kubernetes играет решающую роль в управлении крипуномасштабными контейнерными приложениями, предлагая несколько преимуществ по сравнению с традиционными механизмами развертывания:

* **Масштабируемость**: автоматически масштабируя количество запущенных контейнеров в зависимости от использования ресурсов и требований приложений, Kubernetes обеспечивает оптимальное использование ресурсов и стабильную производительность приложений.
* **Самовосстановление**: Kubernetes постоянно контролирует состояние ваших контейнеров и заменяет вышедшие из строя podы для поддержания требуемого состояния приложения.
* **Постепенные обновления и откаты**: Kubernetes упрощает обновление ваших приложений путем постепенного развертывания новых версий образов контейнеров без каких-либо простоев.
* **Балансировка нагрузки**: сервисы в Kubernetes распределяют сетевой трафик между экземплярами контейнеров, предлагая решение по балансировке нагрузки для ваших приложений.

#### Что выбрать? Kubernetes или Docker Swarm

Хотя Kubernetes и Docker Swarm являются платформами оркестрации, они различаются по сложности, масштабируемости и простоте использования. Kubernetes предоставляет более продвинутые функции, лучшую масштабируемость и более высокую отказоустойчивость, но требует более крутой кривой обучения. Docker Swarm, с другой стороны, проще и понятнее, но ему не хватает некоторых расширенных функциональных возможностей.

В контексте этих различий выбор правильной платформы оркестрации зависит от потребностей и требований вашего проекта.

### Docker Swarm

Docker Swarm – это инструмент оркестрации контейнеров, который позволяет пользователям управлять несколькими узлами Docker и развертывать на них сервисы. Это встроенная в Docker Engine функция кластеризации и оркестрации, которая позволяет создавать и управлять множеством узлов Docker, называемых *Swarm*.

#### Ключевые понятия

* **Узел**: Узел Docker – это экземпляр Docker Engine, который участвует в *Swarm*. Узлы могут быть либо рабочими, либо менеджерами. Рабочие узлы отвечают за запуск контейнеров, тогда как узлы-менеджеры управляют *Swarm* и хранят необходимые метаданные.

* **Сервисы**. Сервис – это высокоуровневая абстракция задач, необходимых для запуска ваших контейнеров. Он определяет желаемое состояние набора контейнеров, указывает Docker образ, желаемое количество реплик и необходимые порты.

* **Задачи**. Задача содержит Docker контейнер и команды, необходимые для его запуска. Узлы-менеджеры *Swarm* назначают задачи рабочим узлам на основе доступных ресурсов.

#### Основные преимущества

* **Масштабируемость**: Docker Swarm позволяет масштабировать сервисы горизонтально, легко увеличивая или уменьшая количество реплик.

* **Балансировка нагрузки**: Swarm гарантирует, что узлы внутри Swarm равномерно обрабатывают рабочие нагрузки контейнера, обеспечивая внутреннюю балансировку нагрузки.

* **Обнаружение сервисов**: Docker Swarm позволяет автоматически обнаруживать другие сервисы в *Swarm*, назначая каждому сервису уникальную запись DNS.

* **Непрерывный процесс обновления**: Swarm позволяет выполнять непрерывные обновления практически с нулевым временем простоя, упрощая процесс развертывания новых версий ваших приложений.

Посетите официальную страинцу [документации Docker Swarm](https://docs.docker.com/engine/swarm/), чтобы узнать больше о его функциях и лучших практиках.
